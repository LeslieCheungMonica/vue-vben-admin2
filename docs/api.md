...
**GET** `/wape/kb_event_stream/{task_id}`

读取 `test_kb_data.json` 文件，替换变量后通过 SSE 推送到前端。
...
```bash
curl -N http://127.0.0.1:7654/wape/kb_event_stream/wape-20250516010101
```

**返回示例：**

```
data: {"type": "kb.retrieval", "properties": {"message": "2025-07-08 15:30:45【INFO】知识库检索命中，相似度：0.8532，匹配片段:【校验集团证件号码与工商信息是否一致...】, 检索耗时: 256 ms ", "task_id": "wape-xxx"}}

data: {"type": "kb.retrieval", "properties": {"message": "2025-07-08 15:30:46【INFO】知识库检索命中，相似度：0.1234，匹配片段:【校验集团证件号码与工商信息是否一致1...】, 检索耗时: 389 ms ", "task_id": "wape-xxx"}}

...

data: {"type": "kb.retrieval.done", "properties": {"message": "知识库检索完成", "task_id": "wape-xxx"}}
```
